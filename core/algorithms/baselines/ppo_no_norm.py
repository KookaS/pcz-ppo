"""ppo_no_norm.py: PPO with no reward or advantage normalization.

Disables both reward normalization (no GDPO, no VecNormalize) and advantage
normalization (``normalize_advantage=False``). This is the most "raw" PPO
baseline, useful for understanding the effect of normalization.

Usage::

    from core.algorithms.ppo_no_norm import PPONoNorm

    model = PPONoNorm("MlpPolicy", env, reward_component_names=["a", "b"])
...
class PPONoNorm(PPO):
    """PPO with no reward or advantage normalization.
...
if __name__ == "__main__":
    run_cartpole_example(PPONoNorm, "PPO No Normalization")
