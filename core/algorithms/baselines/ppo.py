"""ppo.py: Standard PPO baseline — no custom reward normalization.

Standard Stable Baselines 3 PPO used as a baseline for comparison with PCZ
variants. Accepts ``reward_component_names`` for API consistency but does not
use them for normalization. Reward normalization should be handled externally
(e.g., via VecNormalize).

Usage::

    from core.algorithms.ppo import StandardPPO

    model = StandardPPO("MlpPolicy", env, reward_component_names=["a", "b"])
...
class StandardPPO(PPO):
    """Standard PPO baseline (no custom reward normalization).

    Collects per-component rewards for logging but uses the original scalar
    reward for GAE advantage computation, identical to SB3's built-in PPO.
    ...
    def collect_rollouts(
        ...
        if not self._reward_component_names:
            # No components — use standard PPO rollout collection
...
if __name__ == "__main__":
    run_cartpole_example(StandardPPO, "Standard PPO Baseline")
