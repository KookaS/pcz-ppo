"""pcz_ppo_vecnorm.py: PCZ-PPO-VecNorm — per-component running std normalization.

Normalizes each reward component by dividing by its running standard deviation
(no mean subtraction), similar to how VecNormalize handles rewards. Running
statistics persist across rollouts via Welford's algorithm.

Usage::

    from core.algorithms.pcz_ppo_vecnorm import PCZPPOVecnorm

    model = PCZPPOVecnorm("MlpPolicy", env, reward_component_names=["a", "b"])
...
class PCZPPOVecnorm(PPO):
    """PCZ-PPO with VecNormalize-style per-component std normalization.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if reward_component_names is None:
            raise ValueError("PCZPPOVecnorm requires reward_component_names.")
    ...
    def collect_rollouts(
        ...
        normalized = buf.component_rewards / safe_std
...
if __name__ == "__main__":
    run_cartpole_example(PCZPPOVecnorm, "PCZ-PPO-VecNorm")
