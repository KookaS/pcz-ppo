...
% Every number below comes from data/results.csv via
% render_claims.py.  \cnum{foo} expands to the
...
% ============================================================
\begin{abstract}
Proximal Policy Optimization (PPO) with Generalized Advantage Estimation (GAE) is the dominant on-policy algorithm for continuous and discrete control tasks. When environments produce composite rewards---a weighted sum of heterogeneous signals with different scales and frequencies---standard aggregate normalization allows high-magnitude components to dominate the learning signal. We adapt the per-component normalization idea from Group reward-Decoupled normalization Policy Optimization (GDPO)~\cite{shao2024gdpo}, originally proposed for critic-free GRPO in language model alignment, to standard PPO with a learned value function and GAE. Our method, \textbf{PCZ-PPO} (Per-Component Z-normalization for PPO), decomposes the reward into $K$ independently normalizable components, applies per-component z-normalization across the rollout buffer, recombines them using environment-defined weights, and feeds the result into standard GAE with a single scalar critic. This decouples reward \emph{scale} (equalized by normalization) from reward \emph{priority} (preserved by weights). We frame PCZ-PPO as a \textbf{method that scales PPO to many reward components}: at 500k steps, PCZ-PPO's mean performance is approximately invariant to $K$ ($\cnum{k4_pcz_stat}$, $\cnum{k6_pcz_stat}$, $\cnum{k8_pcz_stat}$ for $K{=}4,6,8$), while standard PPO degrades progressively ($\cnum{k4_ppo_stat}$, $\cnum{k6_ppo_stat}$, $\cnum{k8_ppo_stat}$). The growing PCZ/PPO gap is driven almost entirely by PPO collapse at high $K$, not by PCZ improvement. Under uncorrected Welch's $t$-tests $K{=}4$ is marginal ($d{=}\cnum{k4_welch_d}$, $p_{\text{raw}}{=}\cnum{k4_welch_p}$, permutation $p{=}\cnum{k4_perm_p}$), $K{=}6$ is the clearest confirmed effect ($d{=}\cnum{k6_welch_d}$, $p_{\text{raw}}{=}\cnum{k6_welch_p}$, permutation $p{=}\cnum{k6_perm_p}$), and $K{=}8$ is on the Welch threshold ($d{=}\cnum{k8_welch_d}$, $p_{\text{raw}}{=}\cnum{k8_welch_p}$, permutation $p{=}\cnum{k8_perm_p}$). Under Holm--Bonferroni correction across the $K{=}\{4,6,8\}$ family, $K{=}6$ retains significance at $\alpha{=}0.05$ (Holm-corrected $p{=}\cnum{k6_holm_p}$), $K{=}8$ is borderline ($p{=}\cnum{k8_holm_p}$), and $K{=}4$ is not significant ($p{=}\cnum{k4_holm_p}$); the strongest confirmed-after-correction claim is therefore at $K{=}6$. Mean-reward ratios (PCZ/PPO) are non-monotonic in $K$ (K=4: \cnum{k4_ratio}$\times$; K=6: \cnum{k6_ratio}$\times$; K=8: \cnum{k8_ratio}$\times$): a small $K{=}4{\to}K{=}6$ dip sits inside the bootstrap CI of either ratio, and the large $K{=}8$ value is inflated by PPO's near-zero mean. The directionally important statement is that PCZ dominance \emph{grows} at high $K$ from PPO degradation, not that the ratio increases strictly monotonically. Cross-seed variance ratios (PPO/PCZ: \cnum{k4_var_ratio}$\times$, \cnum{k6_var_ratio}$\times$, \cnum{k8_var_ratio}$\times$) favour PCZ-PPO at $K{=}4$ and $K{=}8$ and are essentially tied at $K{=}6$. With matched TorchRL infrastructure trained to 4M steps on $K{=}4$ ($n{=}\cnum{w10_k4_4M_pcz_seeds}$ per arm, extended from $n{=}3$), PCZ-PPO retains a directional mean advantage over PPO (PCZ-PPO $\cnum{w10_k4_pcz_stat}$ vs PPO $\cnum{w10_k4_ppo_stat}$, $\Delta{=}\cnum{w10_k4_4M_delta}$, PPO/PCZ cross-seed variance ratio $\cnum{w10_k4_4M_var_ratio}{\times}$). PPO partially narrows but does not fully close the 500k-step gap by 4M, and PCZ-PPO's variance advantage persists at plateau. PCZ-PPO is therefore most valuable in compute-constrained regimes where reaching a given performance level with fewer samples is itself the goal, and at plateau where the variance advantage supports deployment reliability. We document a sharp usage constraint at $K{=}8$ over 4M steps. Under the env-default \emph{equal} weights, PCZ-PPO at $K{=}8$/4M collapses to $\cnum{rr16_equal_k8_4M_pcz_stat}$ while matched-infrastructure PPO holds at $\cnum{rr16_equal_k8_4M_ppo_stat}$ ($n{=}\cnum{rr16_equal_k8_4M_pcz_seeds}$, $\Delta{=}\cnum{rr16_equal_k8_4M_delta}$); a "SNR-informed" alternative that zeros the noisiest component reproduces the collapse ($\cnum{rr16h8_k8_4M_pcz_stat}$ vs $\cnum{rr16h8_k8_4M_ppo_stat}$, a zero-weighted-channel variant). PCZ-PPO at $K{=}8$/4M only beats PPO when the user supplies hand-designed heterogeneous K-analogous weights $(10,1,1,1,1,1,0.5,0.5)$: $\cnum{w10_k8_pcz_stat}$ vs $\cnum{w10_k8_ppo_stat}$ ($n{=}3$, $\Delta{=}\cnum{rr16h7_k8_4M_delta}$, $\cnum{rr16h7_k8_4M_var_ratio}{\times}$ lower variance). Practical implication: PCZ-PPO at high $K$ and long horizons \emph{requires careful weight selection} to outperform PPO; the headline $K{=}8$ win at 500k does not transfer to 4M for free (§\ref{sec:limitations}). A comprehensive ablation suite confirms that per-component decomposition, running statistics, and post-normalization weights are all essential ingredients, and PCZ-PPO requires no architectural changes to PPO---only a preprocessing step on the reward buffer.
\end{abstract}
...
We evaluate PCZ-PPO in two domains: traditional RL (continuous/discrete control) and LLM alignment (RLHF). Table~\ref{tab:overview} summarizes the key results across both domains.

\begin{table}[H]
\centering
\caption{Overview of PCZ-PPO results across domains. Traditional RL provides the primary evidence; LLM alignment extends to the original GDPO domain. ``Benefit'' indicates whether PCZ-PPO improves over the domain-appropriate baseline. Results marked with $\dagger$ are preliminary.}
\label{tab:overview}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llccl}
\toprule
Domain & Environment & $K$ & PCZ/Baseline & Verdict \\
\midrule
\multirow{4}{*}{Traditional RL} & LunarLander & 4 & $\cnum{k4_ratio}\times$ ($p_{\text{raw}}{=}\cnum{k4_welch_p}$) & Marginal (Holm $p{=}\cnum{k4_holm_p}$) \\
 & LunarLander-K6 & 6 & $\cnum{k6_ratio}\times$ ($p_{\text{raw}}{=}\cnum{k6_welch_p}$) & Strongest (Holm $p{=}\cnum{k6_holm_p}$) \\
 & LunarLander-K8 & 8 & $\cnum{k8_ratio}\times$ ($p_{\text{perm}}{=}\cnum{k8_perm_p}$) & Wins 500k; 4M needs hand-tuned weights (§\ref{sec:limitations}) \\
 & HalfCheetah & 2 & $\cnum{ceHC_ratio}\times$ & Parity \\
 & Reacher (neg.\ control) & 4 & $1.0\times$ & Parity (expected) \\
\midrule
LLM Alignment & PPO Qwen2.5-0.5B & 6 & 200-step PCZ tight variance ($\cnum{LLMppo_k6_var_ratio}{\times}$ lower) reverses at 500 steps to PCZ higher variance + lower mean & Transient-only ($n{=}\cnum{LLMppo_k6_standard_seeds}$/$\cnum{LLMppo_500_k6_standard_seeds}$, §\ref{sec:llm_alignment}) \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
\multicolumn{9}{l}{\textit{\textbf{A15} (symlog$\to$z-norm) variant vs.\ A1 (running) per $K$ --- full analysis in \S\ref{sec:limitations}}} \\
2 & $\cnum{k2_symznorm_stat}$ & \multicolumn{7}{l}{vs A1: $\cnum{k2_pcz_stat}$ ($n{=}\cnum{k2_symznorm_seeds}$); directional, no inferential test at $n{=}3$} \\
4 & $\cnum{k4_symznorm_stat}$ & \multicolumn{7}{l}{\textbf{A15 wins}: $\Delta{=}\cnum{abl_a15_over_a1_delta}$ vs A1, $p{=}\cnum{abl_a15_over_a1_welch_p}$} \\
6 & $\cnum{k6_symznorm_stat}$ & \multicolumn{7}{l}{tied: $\Delta{=}\cnum{abl_a15_over_a1_k6_delta}$ vs A1, paired $p{=}\cnum{abl_a15_over_a1_k6_paired_p}$} \\
\multicolumn{9}{l}{\quad $K{=}8$: \textit{A1 beats A15} ($\Delta{=}\cnum{abl_a15_over_a1_k8_delta}$, $p{=}\cnum{abl_a15_over_a1_k8_welch_p}$) --- prefer A1 at $K{\geq}8$} \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
A1 & PCZ-PPO (running) & $\mathbf{\cnum{ablA1_stat}}$ & \cnum{ablA1_seeds} & \textbf{Reference config ($n{=}10$)} \\
A8 & PCZ-PPO (ZCA whitening) & $\cnum{ablA8_stat}$ & \cnum{ablA8_seeds} & Within one SD of A1, variance ratio $\cnum{abl_zca_var_ratio}\times$ \\
A15 & PCZ-PPO (symlog $\to$ z-norm) & $\cnum{ablA15_stat}$ & \cnum{ablA15_seeds} & \textbf{Beats A1 at K=4} ($\Delta{=}\cnum{abl_a15_over_a1_delta}$, $d{=}\cnum{abl_a15_over_a1_welch_d}$); inverts at K=8 \\
A6 & PCZ-PPO (no adv.\ whitening) & $\cnum{ablA6_stat}$ & \cnum{ablA6_seeds} & Whitening \emph{needed}: 2/10 seeds collapse \\
A7 & PPO-weighted-running & $\cnum{ablA7_stat}$ & \cnum{ablA7_seeds} & Decomposition adds $\cnum{abl_decomp_delta}$ \\
C5 & Multi-head PPO (v2) & $\cnum{ablC5_stat}$ & \cnum{ablC5_seeds} & Value decomp.\ helps, not enough \\
A5 & PPO z-norm-post & $\cnum{ablA5_stat}$ & \cnum{ablA5_seeds} & Unstable (s44 collapse) \\
B1 & PPO (baseline) & $\cnum{ablB1_stat}$ & \cnum{ablB1_seeds} & Several seeds collapse to negative returns \\
B4 & PPO aggregate z-norm & $\cnum{ablB4_stat}$ & \cnum{ablB4_seeds} & Decomposition matters \\
\midrule
C1 & GRPO (no critic) & $\cnum{ablC1_stat}$ & \cnum{ablC1_seeds} & Critic-free fails ($n{=}3$) \\
D1 & PCZ-PPO-MC (no critic)\footnotemark[3] & $\cnum{ablD1_stat}$ & \cnum{ablD1_seeds} & Critic essential for PCZ ($n{=}\cnum{ablD1_seeds}$) \\
   & PCZ-GRPO (ours, no critic)\footnotemark[2] & $\cnum{ablGDPO_stat}$ & \cnum{ablGDPO_seeds} & PCZ further hurts critic-free ($n{=}3$) \\
...
\caption{Left: PCZ-PPO learns faster and converges higher at all timestep budgets. Right: mean reward gap (PCZ$-$PPO) and variance ratio (PPO$/$PCZ). 100k/200k/1M points use $n{=}3$ seeds; 500k uses $n{=}10$. The mid-training variance spike at 200k is driven by 3-seed PPO scatter (some seeds learning, some still below 0) and should be read qualitatively, not as a tight estimate.}
...
LunarLander-K2 & 2 & Coarse (2 components) & \cnum{k2_ratio}$\times$ & \cnum{k2_pcz_seeds} & PCZ wins ($d{=}\cnum{k2_welch_d}$, $n{=}5$) \\
\midrule
...
We ran three LLM regimes on Qwen2.5-0.5B~\cite{qwen2024} + 4-bit QLoRA~\cite{dettmers2023qlora} to check whether the gym findings transfer. All runs evaluate on 20 held-out prompts with the raw composite reward.
...
\textbf{(4) Legacy K{=}2 RLOO, 200 steps, $n{=}3$}: Both modes eval-floor at 0 as previously reported (Standard $\cnum{LLM_k2_standard_stat}$ vs PCZ $\cnum{LLM_k2_pcz_stat}$, $\Delta{=}\cnum{LLM_k2_delta}$). Superseded by (1)-(3) above: $K{=}2$ is uninformative because \texttt{format\_rating} never fires, reducing the cell to effective $K{=}1$. Retained for completeness.

\textbf{(5) 2$\times$2 factorial at $K{=}4$, 200 steps, $n{=}\cnum{LLMfact_k4_seeds}$ per cell (disentangles critic from normalization).} To test whether the critic or the normalization is the dominant driver, we ran all four combinations: \{critic-equipped PPO, critic-free RLOO\} $\times$ \{standard multi-reward, per-component $z$-norm\}. Results: RLOO-standard $\cnum{LLMfact_k4_rloo_standard_stat}$, RLOO-PCZ $\cnum{LLMfact_k4_rloo_pcz_stat}$, PPO-standard $\cnum{LLMfact_k4_ppo_standard_stat}$, PPO-PCZ $\cnum{LLMfact_k4_ppo_pcz_stat}$. \textbf{The critic is the dominant driver} (main effect PPO$-$RLOO $\Delta{=}\cnum{LLMfact_k4_main_critic}$, Welch $p{=}\cnum{LLMfact_k4_main_critic_p}$); \textbf{PCZ is null within both critic settings} (main effect PCZ$-$standard $\Delta{=}\cnum{LLMfact_k4_main_pcz}$, $p{=}\cnum{LLMfact_k4_main_pcz_p}$). This reverses the speculative Cycle-48 prior that the critic would be underutilized in single-step (critic actually helps substantially here at 200 steps) and confirms that PCZ adds no benefit on single-step LLM alignment regardless of whether a critic is used. Combined with (1)--(3), the LLM scope-boundary is clear: PCZ-PPO's per-component normalization mechanism does not generalize to single-step preference-alignment tasks; the only directional signal is the $K{=}6$ cross-seed variance reduction under a critic (2).

\textbf{LLM verdict.} With the 500-step extension showing PCZ's 200-step advantage reversing, the LLM-scale picture is \emph{null to negative}: PCZ does not help the critic-equipped PPO case at $K{=}4$ (regime 1) or at $K{=}6$ (regime 2, when trained long enough), and the 200-step variance-reduction signature on $K{=}6$ is a short-horizon artifact that does not generalize. Critic-free RLOO at this eval setup exhibits a structural train/eval mismatch that precludes normalization-mechanism comparisons (regime 3). The 2$\times$2 factorial (regime 5) further confirms that PCZ is null under both critic and critic-free single-step alignment. We present these findings as an honest scope-boundary for PCZ-PPO: the per-component normalization mechanism does not transfer to single-step preference-alignment with this reward-stack shape on this model scale; multi-turn or process-reward settings remain open (§\ref{sec:future}). Peak VRAM 6.9\,GB for PPO-critic (critic-head + QLoRA), 1.06\,GB for RLOO (QLoRA only).

\textbf{Logging-units correction (retained).} An earlier revision of this paper reported Standard RLOO $\approx\!+2.7$ and PCZ-RLOO $\approx\!-0.1$ as ``final training reward'', concluded PCZ-RLOO \emph{collapses} ($\Delta{\approx}-2.8$), and offered a contextual-bandit interpretation for that sign. That comparison was invalid: TRL's \texttt{reward} log entry records whatever the reward functions return, which in standard mode is the raw weighted sum (units: reward points) but in PCZ mode is the \emph{z-normalised} composite (units: standard deviations about the running EMA mean, zero by construction). On the same raw eval functions both modes produce the same output distribution at the 200-step RLOO budget. We retain the contextual-bandit interpretation of PCZ's negative effect in critic-free gym settings (§\ref{sec:ablation}: PCZ-GRPO vs plain GRPO at matched $n{=}3$, $\cnum{abl_pcz_hurts_grpo_delta}$) because that comparison is in comparable units.
...
A potential concern is that PCZ-PPO's advantage stems from hyperparameter tuning rather than the normalization method. We address this with a targeted PPO tuning sweep: 5 entropy values $\times$ 3 learning rates $\times$ 2 seeds = 30 PPO runs at LunarLander $K{=}4$ 500k, canonical weights. The sweep identified PPO's peak cell as $\mathrm{lr}{=}3\mathrm{e}{-}4$, $\mathrm{ent}{=}0.0$, at which PPO achieves $\cnum{w7_ppo_best_stat}$ ($n{=}\cnum{w7_ppo_best_seeds}$)---well above PPO's canonical cosine-schedule performance of $\cnum{k4_ppo_stat}$ ($n{=}\cnum{k4_ppo_seeds}$). At PPO's own best cell, we then re-run PCZ-PPO at the same $\mathrm{lr}{=}3\mathrm{e}{-}4$, $\mathrm{ent}{=}0.0$ ($n{=}\cnum{w7_pcz_best_seeds}$), which achieves $\cnum{w7_pcz_best_stat}$: a peak-vs-peak gap of $\cnum{w7_delta_best}$. So even when PPO is given its own preferred tuning, PCZ-PPO exceeds it; the full canonical-schedule gap of $\cnum{k4_delta}$ reflects PCZ-PPO's wider tuning envelope rather than a uniquely favourable choice of hyperparameters against PPO. This peak-vs-peak comparison is at $n{=}\cnum{w7_pcz_best_seeds}$ per cell (extended from $n{=}2$ in an earlier revision) --- strong enough to support the tuning-fairness claim directionally, with the canonical $n{=}10$ cosine-schedule cell remaining the inference-ready headline statistic. An earlier 1M-step exploratory test of PPO with LR annealing and cosine entropy $0.1 \to 0.03$ similarly did not outperform the standard 1M PPO ($\cnum{k4_1M_ppo_stat}$).
...
\midrule
A9 & PCZ-PPO (symlog, no z-norm) & $\cnum{ablA9_stat}$ & \cnum{ablA9_seeds} & Matches A1 (alt.\ to running z-norm) \\
A10 & PCZ-PPO (quantile / rank) & $\cnum{ablA10_stat}$ & \cnum{ablA10_seeds} & Fails: rank discards priority-bearing magnitudes \\
A11 & PCZ-PPO (per-component GAE $\lambda_k$) & $\cnum{ablA11_stat}$ & \cnum{ablA11_seeds} & Fails: per-$\lambda$ advantages diverge with shared critic \\
A12 & PCZ-PPO (asymmetric sign-split) & $\cnum{ablA12_stat}$ & \cnum{ablA12_seeds} & Fails: sign-conditional stats break reward scale \\
A13 & PCZ-PPO (min-max Tchebycheff) & $\cnum{ablA13_stat}$ & \cnum{ablA13_seeds} & Fails: policy optimises only the worst component each step \\
A14 & PCZ-PPO (MI-proxy variance weighting) & $\cnum{ablA14_stat}$ & \cnum{ablA14_seeds} & Underperforms: variance ratio over-amplifies volatile components \\
A16 & PCZ-PPO (cosine weight annealing) & $\cnum{ablA16_stat}$ & \cnum{ablA16_seeds} & Matches A1: weight initialisation does not bottleneck PCZ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{K=2 behaviour:} With only 2 components (\texttt{landing} $\pm100$ + \texttt{dense} aggregate), PCZ-PPO still outperforms PPO ($\cnum{k2_pcz_stat}$ vs $\cnum{k2_ppo_stat}$, $d{=}\cnum{k2_welch_d}$, Welch $p_{\text{raw}}{=}\cnum{k2_welch_p}$, $n{=}5$). The \texttt{landing} component's per-step $\sigma/|\mu|{=}35.8$ is still enough to dominate aggregate running statistics even against a single coarse dense channel. K=2 is a 5-seed supporting-evidence cell and is not part of the Holm family; we do not promote it to headline status, but it does not support the earlier "aggregate normalization already suffices" framing either.
...
\item \textbf{K=4 PPO seed sensitivity:} PPO's K=4 performance is highly seed-dependent. At 10 seeds, 4 out of 10 PPO seeds fall below reward 100 (seeds 45, 47, 48, 51), while 0 out of 10 PCZ-PPO seeds do. The 100-threshold is not pre-registered; we pick it as LunarLander's visually-natural break between hovering agents and landing agents. A Mann--Whitney $U$ test on the continuous eval distribution gives $U{=}91$, $p{=}0.005$ (two-sided) without requiring a threshold, supporting the conclusion under a distribution-free test as well.
\end{itemize}
...
\textbf{Robustness checks (BCa, IQM, stochastic dominance).} At $n{=}10$ with heavy-tailed distributions (notably $K{=}8$ PPO), basic-percentile coverage can be optimistic. We therefore re-compute each headline quantity with three complementary tools and report all of them; the paper's main claims hold only if every check agrees. \emph{(i) BCa bootstrap.} Efron's bias-corrected and accelerated bootstrap (10\,000 resamples, seed 20260416) adjusts for skew and median bias; BCa 95\% CIs on the mean difference: $\cnum{k4_bca_ci}$ ($K{=}4$), $\cnum{k6_bca_ci}$ ($K{=}6$), $\cnum{k8_bca_ci}$ ($K{=}8$). All three exclude zero, and the $K{=}8$ interval shifts rightward relative to percentile, confirming the percentile CI was slightly conservative on the tail side. \emph{(ii) IQM per Agarwal et al.~\cite{agarwal2021rliable}.} Interquartile mean drops the two most extreme of ten scores and averages the middle six; at $n{=}10$ this is resistant to single-seed outliers. IQM point estimates (stratified bootstrap 95\% CI, 10\,000 resamples): PCZ-PPO $\cnum{k4_pcz_iqm}$ CI $\cnum{k4_pcz_iqm_ci}$ vs PPO $\cnum{k4_ppo_iqm}$ CI $\cnum{k4_ppo_iqm_ci}$ at $K{=}4$; PCZ-PPO $\cnum{k6_pcz_iqm}$ CI $\cnum{k6_pcz_iqm_ci}$ vs PPO $\cnum{k6_ppo_iqm}$ CI $\cnum{k6_ppo_iqm_ci}$ at $K{=}6$; PCZ-PPO $\cnum{k8_pcz_iqm}$ CI $\cnum{k8_pcz_iqm_ci}$ vs PPO $\cnum{k8_ppo_iqm}$ CI $\cnum{k8_ppo_iqm_ci}$ at $K{=}8$. The PCZ IQM exceeds the PPO IQM at every $K{\in}\{4,6,8\}$, with $K{=}8$ showing the widest separation and the largest PPO CI (consistent with the Welch/BCa analyses). \emph{(iii) Stochastic dominance.} Vargha--Delaney $A_{12}{=}P(X{>}Y)$ with 0.5 weight on ties (a formal test rather than an IQM-figure read-off): $A_{12}{=}\cnum{k4_prob_sup}$ ($K{=}4$), $\cnum{k6_prob_sup}$ ($K{=}6$), $\cnum{k8_prob_sup}$ ($K{=}8$); Mann--Whitney $U$ two-sided $p$-values $\cnum{k4_mwu_p}$, $\cnum{k6_mwu_p}$, $\cnum{k8_mwu_p}$ respectively. The mean-based, BCa, IQM and rank-based tests agree on the direction at every $K$, and all agree on significance at $K{\in}\{6,8\}$ (BCa CI excludes zero; MWU $p{<}0.05$; non-overlapping IQM CIs at $K{=}6$). At $K{=}4$ the MWU $p$ is 0.089 and IQM CIs overlap, consistent with the Welch/Holm marginality already disclosed in Table~\ref{tab:kscaling} --- we do not claim tight $K{=}4$ significance from any single test.
...
\midrule
Seeds (headline) & 42--51 ($n{=}10$) at $K{\in}\{4,8\}$ & chronologically-latest dedupe \\
 & 42--56 ($n{=}15$) at $K{=}6$ & $n{=}10$ pre-registered + 5 robustness \\
